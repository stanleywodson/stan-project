...
export default function User({ auth, users, permissions, flash }: PageProps<{ permissions: Permissions[] }>) {
    ...
    const handleAttachPermission = (userId: number | undefined, permissionId: number) => {
            ...
            <div className="mt-2">
                <div className="max-w-8xl mx-auto">
                    <div className="relative overflow-x-auto shadow-sm mt-4">
                        {flash.attach && (
                            <div className="bg-green-600 text-white border rounded-lg border-gray-500 inline-block mb-2 p-2">
                                {flash.attach}
                            </div>
                        )}
                        ...
                        <div className="flex-1">
                            ...
                            <ul className="mt-2 items-center w-full text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg sm:flex dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                {permissionAvaliable?.map(({ name, id }) => (
                                    <Link href={route('permission-user-atach', [user?.id, id])}>
                                        <li className={`w-full border-b border-gray-200 sm:border-b-0 sm:border-r dark:border-gray-600`}>
                                            <div className="flex items-center ps-3">
                                                <input onChange={() => handleAttachPermission(user?.id, id)} id={name} type="checkbox" value="" className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500" />
